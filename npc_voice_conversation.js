...
// Backend URL can be overridden at runtime by creating a ".env" file
// served at the web root with a line like: BACKEND_URL=http://localhost:8000
// Or set `window.__BACKEND_URL__` before this script runs.
...
window.addEventListener("beforeunload", (event) => {
  if (isProcessingVoice) {
    ...
    event.returnValue =
      "Voice processing in progress. Are you sure you want to leave?";
  }
});
...
function initVoiceSystem() {
  ...
  recognition.onresult = (event) => {
    ...
    for (let i = 0; i < result.length; i++) {
      const alt = result[i];
      console.log(
        `   Alternative ${i + 1}: "${alt.transcript}" (${(alt.confidence * 100).toFixed(0)}%)`,
      );
    ...
    console.log(
      `🎤 FINAL SEGMENT: "${transcript}" (confidence: ${(bestConfidence * 100).toFixed(0)}%)`,
    );

    // ACCUMULATE - don't stop, keep listening until V pressed
    if (transcript && bestConfidence > 0.2) {
      // Lower threshold for better pickup
...
function showRecordingIndicator() {
  ...
  if (el) {
    el.innerHTML =
      '<div class="recording-dot"></div>🎤 பேசுங்கள்... (V ஐ அழுத்தவும்)';
