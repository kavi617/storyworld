...
export function initTempleWorld(scene) {
    ...
    southWallLeft.position.set(-complexSize/4 - 5, wallHeight/2, -complexSize/2);
    ...
    southWallRight.position.set(complexSize/4 + 5, wallHeight/2, -complexSize/2);
    ...
    textPlane.position.set(0, archHeight + 1.5, -complexSize/2 - archDepth/2 - 0.01); // On front face of lintel
    ...
    console.log('✓ Tamil cultural elements added: Kolam patterns, Vilakku (oil lamps), side shrines, garlands');
    ...
    northWater.position.set(0, waterY, complexSize/2 + waterWidth/2 + waterGap);
    ...
    eastWater.position.set(complexSize/2 + waterWidth/2 + waterGap, waterY, 0);
    scene.add(eastWater);
    ...
    westWater.position.set(-complexSize/2 - waterWidth/2 - waterGap, waterY, 0);
    scene.add(westWater);
    environmentObjects.push(westWater);
    
    console.log('✓ Temple ground, compound walls (Prakara), and outer water bodies created');
    ...
    loader.load(
        ...
        (fbx) => {
            ...
            console.log(`   🔧 Auto-scaled temple by ${scaleFactor.toFixed(6)}x (from ${maxDimension.toFixed(0)} to ${targetMaxSize} units)`);
            ...
            console.log('✓ Temple mesh in groundMeshes only — player can enter interior freely');
            console.log('  Temple footprint X:', finalBox.min.x.toFixed(1), 'to', finalBox.max.x.toFixed(1));
            console.log('  Temple footprint Z:', finalBox.min.z.toFixed(1), 'to', finalBox.max.z.toFixed(1));
            ...
            const poleGeometry = new THREE.CylinderGeometry(poleRadius, poleRadius * 1.2, poleHeight, 12);
            ...
            console.log('✓ Realistic Nandhi in Nandi Mandapam (facing entrance) at z=' + (entranceZ - 18).toFixed(1));
            console.log('✓ Kodi Kampam (flagpole) at z=' + (entranceZ - 30).toFixed(1));
            ...
            // Transparent background
            tripundraCtx.clearRect(0, 0, tripundraCanvas.width, tripundraCanvas.height);
            ...
            console.log('✓ Shiva Lingam with Tripundra and Kumkum markings added in temple sanctum');
            ...
            console.log('   📸 Recommended camera: position at (0, ' + (finalSize.y * 0.5).toFixed(1) + ', ' + (finalSize.z * 2).toFixed(1) + '), look at (0, ' + (finalSize.y * 0.4).toFixed(1) + ', 0)');
            console.log('═══════════════════════════════════════════════════════════');            
        ...
        (error) => {
            ...
            console.error('   Check if file exists at: Theme/Temple Theme/temple.fbx');
            console.error('═══════════════════════════════════════════════════════════');
        }
    );
...
function _LoadRajaNPC(scene) {
    ...
    loader.load(
        ...
        (fbx) => {
            ...
            fbx.position.set(4, 5, -40.3);  // 13 more units forward // raja raja cholan model position
...
export function removeTempleWorld(scene) {
    ...
    scene.traverse((child) => {
        if (child.name === 'TempleAmbientLight' || child.name === 'TempleSunLight') {
